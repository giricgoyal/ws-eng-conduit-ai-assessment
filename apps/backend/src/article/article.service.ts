...
import { Tag } from 'apps/backend/src/tag/tag.entity';
...
export class ArticleService {
  ...
  async update(userId: number, slug: string, articleData: any): Promise<IArticleRO> {
    ...
    if (articleData.coAuthors && Array.isArray(articleData.coAuthors)) {
      const coAuthorUsers = await this.userRepository.find({
        email: { $in: articleData.coAuthors.map((email: string) => email.trim()) },
      });

      article?.coAuthors.set(coAuthorUsers);
      delete articleData.coAuthors; // Remove it from the data so it will not be re-assigned by MikroORM
    }
